...
#Remove 178,890 duplicate rows
Nutrition2022_2023_nodups <- Nutrition2022_2023 %>%
  distinct(.keep_all = TRUE)


#rename Purchase Number and Week No variables in nutrition file to match master file
colnames(Nutrition2022_2023_nodups) [colnames(Nutrition2022_2023_nodups) %in% c("Purchase Number", "Week No")] <- c("purchnum", "weeknum")

#recode purchnum as numeric
HFSSFINAL22_23$purchnum <- as.numeric(HFSSFINAL22_23$purchnum)


#Link combined nutrition file to master file using purchase number and week number variables
HFSSFINAL22_23 <- HFSSFINAL22_23 %>%
  left_join(Nutrition2022_2023_nodups, by = c("purchnum", "weeknum"))

summary(HFSSFINAL22_23)
...
Croissants <- HFSSFINAL22_23NUT %>% 
  filter(HFSSFINAL22_23NUT$purchnum == '3140021' | purchnum == '3958225')
